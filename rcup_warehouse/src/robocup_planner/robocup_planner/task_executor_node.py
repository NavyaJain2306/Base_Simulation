...
Coordinates the warehouse robot between the task planner and the
navigation node. Execution runs in a background thread so ROS callbacks
keep being processed while the robot is navigating.

Subscribes to:
  /planned_task         (sml_messages/Task)
  /go_to_shelf_result   (std_msgs/String)

Publishes to:
  /go_to_shelf_cmd      (std_msgs/String)
"""
...
class TaskExecutorNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("TaskExecutorNode started. Waiting for /planned_task ...")
    ...
    def _on_planned_task(self, msg: PlannedTask):
        ...
        threading.Thread(target=self._run_execution, args=(msg,), daemon=True).start()
    ...
    def _run_execution(self, msg: PlannedTask):
        ...
        for order in msg.order_list:
            self._execute_order(order, storage_map, workbench)

    def _execute_order(self, order, storage_map, workbench):
        ...
        if order.order_type == Order.OT_PRODUCE:
            for bt in block_sequence:
                ...
                if station is None:
                    self.get_logger().error(f"No storage station for block type {bt}. Skipping.")
                    continue
        ...
        self.get_logger().info(f"Order product_id={product_id} completed successfully.")

    def _navigate_to(self, station_name: str, reason: str = "") -> bool:
        self.get_logger().info(
            f'  => navigating to: "{station_name}"' + (f"  ({reason})" if reason else "")
        )
        ...
        if not arrived:
            self.get_logger().error(f'TIMEOUT waiting for result from "{station_name}"')
            return False
        ...
        if self._nav_result == f"{station_name}:success":
            ...
            self.get_logger().error(f'  => FAILED for "{station_name}": {self._nav_result}')
            return False

    def _build_storage_map(self, stations):
        """Map block type -> storage station name."""
        storage_map = {}
