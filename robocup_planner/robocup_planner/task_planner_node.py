"""
Task Planner Node - SML EAI Workshop (v3)
==========================================
...
Dependencies:
  ...
  ROS2 with sml_messages built in your workspace
"""
...
from sml_messages.msg import Task, Order, Station, PlannedTask, PlanStep
...
class TaskPlannerNode(Node):
    ...
    def __init__(self):
        ...
        self.inventory: dict[int, list[str]] = {}   # block_type -> [station_names]
        self.arena_stations: list[Station] = []

        self.get_logger().info('TaskPlannerNode v5 started. Waiting for /task ...')
    ...
    def task_callback(self, msg: Task):
        self.get_logger().info(
            f'Received task: {len(msg.order_list)} orders, '
    ...
    def _build_storage_map(self) -> dict[int, str]:
        """block_type -> first storage station that has it"""
    ...
    def _build_shelf_occupancy(self) -> dict[str, int]:             # for smart RECYCLING process
        """Count how many blocks each storage shelf currently holds"""
        occupancy = {}
    ...
    def _least_occupied_shelf(self, occupancy: dict[str, int]) -> str:
        """Return shelf name with fewest blocks"""
        return min(occupancy, key=occupancy.get)

    def _build_recycle_return_map(
        self,
        block_sequence: list[int],
        occupancy: dict[str, int]
    ) -> dict[int, str]:
        """
        For each block type in sequence, assign the least occupied shelf.
        Increment occupancy after each assignment so next block goes elsewhere.
        Returns {block_type: shelf_name}
        """
        return_map: dict[int, str] = {}
        # Work on a copy so we don't modify the original
        occ = dict(occupancy)
        for bt in block_sequence:
            shelf = self._least_occupied_shelf(occ)
            return_map[bt] = shelf
            occ[shelf] += 1
        return return_map
    ...
    def _solve_with_up(self, orders: list[Order]) -> tuple | None:
        ...
        robot_free         = Fluent('robot_free',         BoolType())           # Nothing on the base- not inbetween picking blocks
        ...
        for f in [block_on_shelf, block_on_base, block_on_workbench,
                  toy_on_workbench, toy_on_base, toy_at_counter,
                  toy_assembled, toy_broken_down,
                  arm_free, robot_free,
                  all_blocks_on_base, all_blocks_on_wb,
                  all_blocks_loaded, order_done, recycle_phase_done]:
        ...
        # In case of no recycling orders 
        ...
        # PRODUCE: fetch block from shelf to bot base
        ...
        fetch_block.add_effect(arm_free, True)   # arm picks and places ends free
        problem.add_action(fetch_block)

        # return_block is grounded per-block per-order (defined inside order loop)

        # ── Metadata dicts ────────────────────────────────────────────────
        block_to_type:  dict[str, int] = {}   # block obj name (blk_o0_b0_t1) -> block_type int (1)
        block_to_shelf: dict[str, str] = {}   # block obj name -> fetch shelf (shelf_a)
        # action_meta: action_name -> (PlanStep constant, block_type, station)
        action_meta: dict[str, tuple] = {} # 'fetch_block(blk_o0_b0_t1)' -> (PlanStep.FETCH_BLOCK, 1, 'shelf_a')

        """
        EXPLAIN - the above three dictionaries exist outside UP problem
                - they are like transition layer, for after planning is done.

                - this is because the planner only gives fetch_block(blk_o0_b0_t1) obj name str
                - we need block type -, type - 1, shelf_a
        """

        storage_map    = self._build_storage_map()      # first shelf having that block eg 1:'shelf_A'
        workbench      = self._get_workbench()
        customer_stn   = 'cc_1'                         # hardcoded cutomer counter
        shelf_occupancy = self._build_shelf_occupancy() # {'shelf_a':3, 'shelf_b':1} - how many blocks are there in each shelf


        """ 
        UP Object Containers - these hold planner representation of things, 
        we keep python reference with them so that latter we can write per order action building loop 
        """
        all_block_objects: dict[str, Object] = {}       # 'blk_o0_b0_t1' -> UP Objects 
        order_objects: list[Object] = []                # Used to set goal - at the end, 
        toy_objects:   list[Object] = []                # Used for building toy-related actions and fluents

        # ── Per-order objects and grounded actions ────────────────────────
        for oi, order in enumerate(orders):     # order index, actial Order
            seq = decode_product(order.product_id)

            ord_obj = Object(f'order_{oi}_pid{order.product_id}', OrderType)    # Creates a UP object eg order_0_pid341, type OrderType
            problem.add_object(ord_obj)
            order_objects.append(ord_obj)

            toy_obj = Object(f'toy_{oi}_pid{order.product_id}', Toy)            # Create a UP objcet eg. 'toy_0_pid341', type Toy
            problem.add_object(toy_obj)
            toy_objects.append(toy_obj)

            order_block_objs = [] #---------------------------------------------# blocks for THIS order only, reset for each order 
            for bi, bt in enumerate(seq):                                       # bi = block index (0,1,2 ...) and bt = block type (3,4,1)
                name = f'blk_o{oi}_b{bi}_t{bt}'                                 # o-> Order, b-> block indes, t-> type
                blk_obj = Object(name, Block)                                   # Create UP object
                problem.add_object(blk_obj)
                all_block_objects[name] = blk_obj
                ...
                if order.order_type == Order.OT_PRODUCE:
                    ...
                    # Toy is at counter, blocks will appear after breakdown
                    problem.set_initial_value(toy_at_counter(toy_obj), True)

                order_block_objs.append(blk_obj)

            # ── PRODUCE per-order grounded actions ────────────────────────
            if order.order_type == Order.OT_PRODUCE:

                # Gate 1: all blocks on base
                ...
                # goto_workbench: drive to wb, arm places all blocks
                ...
                goto_wb.add_effect(all_blocks_on_base(ord_obj), False)  # reset gate
                ...
                # load_toy: arm picks assembled toy from wb, places on base
                ...
                # deliver: drive to cc_1, arm places toy
            ...
            # ── RECYCLE per-order grounded actions ────────────────────────
            elif order.order_type == Order.OT_RECYCLE:

                # fetch_recycle: go to cc_1, arm picks toy, places on base
                ...
                # place_recycle: drive to wb, arm places toy, toy breaks down
                ...
                # load_blocks: arm picks each block from wb onto base
                ...
                # return_block per block grounded, least occupied shelf
                # keyed by bi (index) not bt (type) to handle duplicate block types
                ...
                for bi, bt in enumerate(seq):
                    ...
                    action_meta[f'return_block_o{oi}_b{bi}_t{bt}()'] = (
                        PlanStep.RETURN_BLOCK, bt, ret_shelf
                    )

                # recycle_done: all blocks returned
        ...
        # Mark recycle_phase_done after last recycle order
                ...
                with OneshotPlanner(name=solver) as planner:
                    ...
                    if result.plan is not None:
                        ...
                        for ai in result.plan.actions:
                            ...
                            # Shared actions keyed by full string
                            action_str = (
                                f'{name}({", ".join(params)})'
                            ...
                            # Shared fetch_block fill meta now (need block obj name)
                            ...
                            # grounded return_block -- meta already set by block name
                            elif name.startswith('return_block_o'):
                                # action_str == 'return_block_oN_bM_tT()'
                                # meta already in action_meta from creation
                                pass
    ...
    # -----------------------------------------------------------------------
    # Build PlanStep list from action sequence
    # -----------------------------------------------------------------------
    def _build_plan_steps(self, action_sequence, action_meta) -> list[PlanStep]:
        ...
        for action_str in action_sequence:
            ...
            if planstep_action == PlanStep.FETCH_BLOCK:
                ...
                step.label = f'drive to {station}, arm place block_type{block_type} on shelf'
