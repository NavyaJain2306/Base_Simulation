"""
Reads block positions from SLAM output.
In competition, SLAM publishes to a ROS topic.
For now, we support:
  - Hardcoded mode (for testing)
  - Dict input mode (when perception guy hands you data)
"""

class SLAMReader:
    def __init__(self, mode="hardcoded"):
        """
        mode = "hardcoded" -> uses built-in test positions
        mode = "live"->call update(data) with real SLAM data
        """
        self.mode = mode
    ...
    def _hardcoded_positions(self):
        ...
        # Hardcoded block positions for testing.
        # Replace these with real values from SLAM during competition.
