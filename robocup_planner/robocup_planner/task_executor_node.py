"""
Task Executor Node - SML EAI Workshop
=====================================================================
Sits between Task Planner and the Nav node (go_to_shelf).

Subscribes to:
  /planned_task         (sml_messages/Task)   - planned task from Task Planner
  /go_to_shelf_result   (std_msgs/String)      - nav result from destination.py

Publishes to:
  /go_to_shelf_cmd      (std_msgs/String)      - station name to navigate to

Execution runs in a background thread so ROS spin() keeps processing
callbacks (especially /go_to_shelf_result) while the robot is navigating.
"""
...
# -----------------------------------------------------------------------
    # Build customer map: block_type -> customer station name
    # from arena_layout ST_CUSTOMER stations
    # -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
    # Get first workbench station name from arena_layout
    # -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
    # Decode product_id into block type list (bottom -> top)
    # e.g. 341 -> [3, 4, 1]
    # -----------------------------------------------------------------------
