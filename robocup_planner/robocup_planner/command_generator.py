...
--------------------
Converts abstract UP plan actions into concrete arm commands.

Each command is a dict:
    { "arm", "x", "y", "z", "roll", "pitch", "yaw", "gripper", "label" }

A single pick or place action expands into 4 movement steps.

Also provides:
    cmd_to_ros_msg(cmd)  ->  converts a command dict to a ROS Pose + gripper string
                            ready to publish on /arm1/command or /arm2/command
"""

LIFT_HEIGHT   = 0.10     # metres to lift above block/target before moving
DEFAULT_ROLL  = 0.0
DEFAULT_PITCH = 1.5708   # 90 deg -- straight down approach
DEFAULT_YAW   = 0.0
...
def cmd_to_ros_msg(cmd: dict):
    """
    Convert a command dict into a ROS-publishable JSON string.

    We use std_msgs/String with JSON payload — no custom message needed.
    The arm node unpacks this on the other side.

    Format published to /arm1/command or /arm2/command:
    {
    ...
    Returns: a plain dict ready to be json.dumps()'d into a String msg.
    """
    return {
        "x":       cmd["x"],
        "y":       cmd["y"],
...
def cmd_to_pose_msg(cmd: dict):
    """
    Convert a command dict into a ROS geometry_msgs/Pose + gripper string.

    Use this if your arm node expects a Pose message rather than JSON.
    Call like:
        pose, gripper = cmd_to_pose_msg(cmd)
        pose_pub.publish(pose)
        gripper_pub.publish(String(data=gripper))

    Requires: from geometry_msgs.msg import Pose, Point, Quaternion
