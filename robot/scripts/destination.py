...
ALIGN_TIMEOUT = 60.0  # seconds to wait for align.py's event_out
...
class GoToShelf(Node):
    ...
    def _result_callback(self, future, target_frame):
        ...
        # --- NEW: nav2 got us close, now hand off to align.py ---
        self.get_logger().info(
    ...
    def _on_align_result(self, msg: String):
        if self._current_target is None:
            return  # stray event_out, nothing in flight
...
#             self.get_logger().error(f'Transform lookup failed: {e}')
#             result_msg = String()
#             result_msg.data = f'{target_frame}:failure'
#             self.result_pub.publish(result_msg)
#             return
...
#         self.result_pub.publish(result_msg)
#         self.get_logger().info(
#             f'Goal {"succeeded" if success else "failed"} for {target_frame}')
